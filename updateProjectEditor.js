...
const newJsx = `  if (!project) return <div className="loading">Cargando proyecto...</div>;
  ...
  return (
    <div className="project-editor modern-editor">
      ...
      <div className="editor-tabs-modern">
        ...
        <button
          className={\`tab-modern \${activeTab === "experiences" ? "active" : ""}\`}
          onClick={() => setActiveTab("experiences")}
        >
          <div className="tab-content">
            <span className="tab-icon"><FaEye /></span>
            <span className="tab-label">Zonas</span>
            <span className="tab-badge">{project.experiences?.length || 0}</span>
          </div>
        </button>
        <button
          className={\`tab-modern \${activeTab === "scenes" ? "active" : ""}\`}
          onClick={() => setActiveTab("scenes")}
        >
          <div className="tab-content">
            <span className="tab-icon"><FaImage /></span>
            <span className="tab-label">Escenas 360°</span>
      ...
      <div className="editor-content-modern" ref={contentRef}>
                ...
                <div className="form-grid-modern">
                  <div className="form-group-modern">
                    ...
                    <input
                      ...
                      placeholder="Ej: ARC Independiente"
                    />
                  </div>
                  <div className="form-group-modern">
                    ...
                    <input
                      type="text"
                      value={project.vesselType || ""}
                      onChange={(e) => handleBasicInfoChange("vesselType", e.target.value)}
                      placeholder="Ej: Patrullero de Costa"
                    />
                  </div>
                  <div className="form-group-modern">
                    <label>Estado</label>
                    ...
                    <div key={doc.id} className="doc-item-modern">
                      ...
                      <div className="doc-info">
                        <input
                          type="text"
                          value={doc.title || ""}
                          onChange={(e) => handleUpdateDocumentTitle(doc.id, e.target.value)}
                          className="doc-input"
                        />
        ...
        {/* EXPERIENCES (ZONAS) TAB */}
        {activeTab === "experiences" && (
          <div className="tab-pane-modern">
            <div className="pane-header">
              <div className="pane-title">
                <h2>Gestión de Zonas</h2>
                <p>Organiza las áreas principales del proyecto.</p>
              </div>
            ...
            <div className="zones-grid-modern">
              {project.experiences?.map((exp, index) => (
                <div key={exp.id} className="zone-card-modern">
                  <div className="zone-card-header">
                    <div className="zone-title-group">
                      <div className="zone-icon-modern">
                        <FaShip /> {/* Fallback icon, could map real icon if needed */}
                      </div>
                      <h3>{exp.name || \`Zona #\${index + 1}\`}</h3>
                    </div>
                    <button className="btn-action-icon danger" onClick={() => handleDeleteExperience(index)}>
                      <FaTrash />
                    </button>
                  </div>
                  ...
                  <div className="zone-card-body">
                    <div className="form-group-modern compact">
                      ...
                      <input type="text" value={exp.id || ""} onChange={(e) => handleUpdateExperience(index, "id", e.target.value)} />
                    </div>
                    <div className="form-group-modern compact">
                      <label>Nombre</label>
                      <input type="text" value={exp.name || ""} onChange={(e) => handleUpdateExperience(index, "name", e.target.value)} />
                    </div>
                    <div className="form-group-modern compact">
                      <label>Icono</label>
                      <select value={exp.icon || "FaShip"} onChange={(e) => handleUpdateExperience(index, "icon", e.target.value)}>
                        {iconOptions.map((icon) => <option key={icon} value={icon}>{icon}</option>)}
                      </select>
                    </div>
                    <div className="form-group-modern compact">
                      <label>Escena Inicial</label>
                      <select value={exp.startScene || ""} onChange={(e) => handleUpdateExperience(index, "startScene", e.target.value)}>
                        <option value="">Seleccionar escena...</option>
                        {Object.keys(project.scenes || {}).map((sceneKey) => (
                          <option key={sceneKey} value={sceneKey}>{project.scenes?.[sceneKey]?.title || sceneKey}</option>
                        ))}
                      </select>
                    </div>
                    <div className="form-group-modern compact full-width">
                      <label>Descripción</label>
                      <textarea value={exp.description || ""} onChange={(e) => handleUpdateExperience(index, "description", e.target.value)} rows="2" />
                    </div>
                  </div>
                </div>
              ))}
        ...
        {activeTab === "scenes" && (
          <div className="tab-pane-modern">
            <div className="pane-header">
              <div className="pane-title">
                <h2>Escenas 360°</h2>
                <p>Administra las panorámicas del proyecto.</p>
              </div>
              <button className="btn-primary-modern" onClick={handleAddScene}>
                <FaPlus /> Añadir Escena
              </button>
            </div>

            <div className="scenes-grid-modern">
              {Object.entries(project.scenes || {}).map(([sceneKey, scene]) => (
                <div key={sceneKey} className="scene-card-modern">
                  ...
                  <div className="scene-card-content">
                    <div className="form-group-modern compact full-width">
                      <label>Título de Escena</label>
                      <input type="text" value={scene.title || ""} onChange={(e) => handleUpdateScene(sceneKey, "title", e.target.value)} />
                    </div>
                    <div className="scene-coords">
                      <div className="form-group-modern compact">
                        <label>Pitch</label>
                        <input type="number" step="0.1" value={scene.pitch ?? 0} onChange={(e) => handleUpdateScene(sceneKey, "pitch", parseFloat(e.target.value))} />
                      </div>
                      <div className="form-group-modern compact">
                        <label>Yaw</label>
                        <input type="number" step="0.1" value={scene.yaw ?? 0} onChange={(e) => handleUpdateScene(sceneKey, "yaw", parseFloat(e.target.value))} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="scene-card-footer">
                    ...
                    <button className="btn-scene-action secondary" onClick={() => setSelectedScene(sceneKey)}>
                      <FaMapMarkerAlt /> Hotspots ({Object.keys(scene.hotSpots || {}).length})
                    </button>
      ...
      {/* FAB & MODALS */}
      {fabConfig && showFab && (
        <div ref={fabRootRef} className={\`pe-fab-modern \${fabOpen ? "open" : ""}\`}>
          <button className="pe-fab-btn-modern" onClick={() => setFabOpen(v => !v)}>
            <FaPlus />
          </button>
        </div>
      )}
...
fs.writeFileSync(filePath, parts[0] + newJsx);
console.log('ProjectEditor.js updated successfully!');
